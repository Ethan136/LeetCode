...
Algorithm:
  ...
  - Hash
TimeCost: "40"
LeetCodeNeedReviewNext:
LeetCodeNeedEnhance:
  - Map搜尋Key和Value都是O(1)可以運用
---
# LeetCode
...
# Process Record
- [x] [[1. Two Sum - Ans 1 - Linear - Sqr(N)]]：20m
	- [x] 想法筆記
	- [x] 實作解題
- [x] [[1. Two Sum - Ans 1 - Hash - N]]: 20m
	- [x] 想法筆記
	- [x] 實作解題
...
public:
    vector<int> twoSum(vector<int>& nums, int target) {
        ...
        for(int i = 0; i < nums.size(); i++) {
            ...
            if(valToIdx.count(needNum)) return {i, valToIdx[needNum]};
            valToIdx[nums[i]] = i;
        }
        return {};
    }
