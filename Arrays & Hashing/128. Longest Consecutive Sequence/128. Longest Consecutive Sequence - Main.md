...
public:
    int longestConsecutive(vector<int>& nums) {
        ...
        for(auto num : num_set) {
            // check consecutive only if the current num is the least num is conseq series
            if(num_set.count(num-1)) continue;

            // check consecutive num cnt
            int consecutive_cnt = 0;
            while(num_set.count(num++)) consecutive_cnt++;

            // update max consecutive cnt
            maxConsecutiveCnt = max( maxConsecutiveCnt, consecutive_cnt );
        }
