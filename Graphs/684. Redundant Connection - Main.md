...
## V3：複習2nd - 模板化寫法（有 path compress）
```Cpp
class Solution {
private:
    int unionFindPivotConnectNode(int node, auto &nodeToConnectNodeInComp) {

        // traverse to pivot connect node
        int travNode = node;
        while(travNode != nodeToConnectNodeInComp[travNode]) {
            travNode = nodeToConnectNodeInComp[travNode];
        }
        int pivotNode = travNode;

        // compress the path nodes to pivot node
        travNode = node;
        while(travNode != nodeToConnectNodeInComp[travNode]) {
            int nextTravNode = nodeToConnectNodeInComp[travNode];
...
public:
    vector<int> findRedundantConnection(vector<vector<int>>& edges) {
        unordered_map<int, int> nodeToConnectNodeInComp;
        for(auto &edge : edges) {
            int node1 = edge[0];
            int node2 = edge[1];

            if(!nodeToConnectNodeInComp.count(node1)) nodeToConnectNodeInComp[node1] = node1;
            if(!nodeToConnectNodeInComp.count(node2)) nodeToConnectNodeInComp[node2] = node2;

            int pivotNode1 = unionFindPivotConnectNode(node1, nodeToConnectNodeInComp);
            int pivotNode2 = unionFindPivotConnectNode(node2, nodeToConnectNodeInComp);

            if(pivotNode1 == pivotNode2) return edge;
            else nodeToConnectNodeInComp[pivotNode1] = pivotNode2;
        }
