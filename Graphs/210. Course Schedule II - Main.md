...
# Answer：複習2nd - 模板化寫法（速度慢，但實作少）
```Cpp
class Solution {
public:
    vector<int> findOrder(int numCourses, vector<vector<int>>& prerequisites) {
        // build course relationships
        unordered_map<int, unordered_set<int>> courseToPreqCourses;
        for(int i = 0; i < numCourses; i++) {
            courseToPreqCourses[i] = {};
        }
        for(auto &courseAndPrereq : prerequisites) {
            courseToPreqCourses[courseAndPrereq[0]].insert(courseAndPrereq[1]);
        }

        // enroll path
        vector<int> enrollSeq;
        enrollSeq.reserve(numCourses);

        // BFS traverse all no-prerequisit courses
        while(courseToPreqCourses.size()) {
            // choose one course no need pre-requisit
            int selectCourse = -1;
            ...
            // if there's no course withour prerequisit
            if(selectCourse == -1) return {};

            // add selected course to enroll seq
            enrollSeq.push_back(selectCourse);

            // take out enrolled course from relationships
