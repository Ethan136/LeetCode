...
# Answer: BFS Traverse - 複習2nd 模板化寫法
```Cpp
class Solution {
public:
    int ladderLength(string beginWord, string endWord, vector<string>& wordList) {
        // make a set of all un-traversed words
        unordered_set<string> allWordSet(wordList.begin(), wordList.end());
        allWordSet.insert(beginWord);

        // check if end word on the list
        if(!allWordSet.count(endWord)) return 0;

        // BFS traverse queue of this / next layer
        queue<string> thisLayerTravWord;
        queue<string> nextLayerTravWord;

        // initial condition
        int wordOnPath = 0;
        thisLayerTravWord.push(beginWord);
        allWordSet.erase(beginWord);

        // BFS traverse
        while(thisLayerTravWord.size()) {

            // update word on path
            wordOnPath++;

            // traverse words on this layer
            while(thisLayerTravWord.size()) {

                // traverse one word
                string travWord = thisLayerTravWord.front();
                thisLayerTravWord.pop();

                // if the traverwd word is the end word
                if(travWord == endWord) return wordOnPath;

                // check word change one by one for next traverse
                for(int i = 0; i < travWord.size(); i++) {
                    char charBkp = travWord[i];
                    for(char c = 'a'; c <= 'z'; c++) {

                        // change one char
                        if(travWord[i] == c) continue;
                        travWord[i] = c;

                        // check if changed word in set & mark traversed by erase
                        if(!allWordSet.count(travWord)) continue;
                        allWordSet.erase(travWord);

                        // push to next traverse
                        nextLayerTravWord.push(travWord);
                    }
                    travWord[i] = charBkp;
                }
            }

            // swap for next layer traverse
            swap(thisLayerTravWord, nextLayerTravWord);
        }

        // finish -> can't traverse to end word
        return 0;
    }
};
```