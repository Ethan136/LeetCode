...
public:
    string longestPalindrome(string s) {
        ...
        auto checkExpandPalindromeLen = [](string &s, int l, int r, int &startIdx) {
            if(l > r) return 0;
        ...
        // try to expand palindrom from each idx
        int maxLen = 0, maxLenStartIdx;
        for(int i = 0; i < s.size(); i++) {
            // single core
            int singleMidStartIdx;
            int singleMidLen = checkExpandPalindromeLen(s, i, i, singleMidStartIdx);

            // double core
            int doubleMidStartIdx;
            int doubleMidLen = checkExpandPalindromeLen(s, i, i+1, doubleMidStartIdx);

            // update max
            if(maxLen < singleMidLen) {
                maxLen = singleMidLen;
                maxLenStartIdx = singleMidStartIdx;
            }
            if(maxLen < doubleMidLen) {
                maxLen = doubleMidLen;
                maxLenStartIdx = doubleMidStartIdx;
            }
        }

        // finish: no aplindrome exist
        return s.substr(maxLenStartIdx, maxLen);
    }
};
```