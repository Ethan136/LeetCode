...
# Answer：DP 模板
**實作起來類似 greedy -> greedy 是 DP 的一種簡化**
```Cpp
...
public:
    int maxProduct(vector<int>& nums) {
        ...
        // two condition
        // (1) max product to idx
        // (2) min product to idx
        vector<int> maxProductToIdx(nums.size());
        vector<int> minProductToIdx(nums.size());
        

		// initial condition
		maxProductToIdx[0] = nums[0];
		minProductToIdx[0] = nums[0];

		// init result
		int maxProductOverall = maxProductToIdx[0];

		// check products along nums[i]
		for(int i = 1; i < nums.size(); i++) {
			maxProductToIdx[i] = max(nums[i], max(minProductToIdx[i-1] * nums[i], maxProductToIdx[i-1] * nums[i]));
			minProductToIdx[i] = min(nums[i], min(minProductToIdx[i-1] * nums[i], maxProductToIdx[i-1] * nums[i]));
			maxProductOverall = max(maxProductOverall, maxProductToIdx[i]);
		}
