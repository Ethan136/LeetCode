...
## V3: DFS + Iteration + Preorder Traverse -> *練習這個*
...
### V3.2: 複習2nd 時候寫的（調整寫法，更直覺化）
```Cpp
class Solution {

public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        // check if setting valid
        if(!preorder.size() || !inorder.size() || (preorder.size() != inorder.size())) return nullptr;

        // un-checked index of inorder & preorder seq
        int preIdx = 0, inIdx = 0;

        // build root
        TreeNode *root = new TreeNode( preorder[ preIdx++ ] );

        // pending node top = traversed latest node
        stack<TreeNode*> preOrdPendingNode;
        preOrdPendingNode.push(root);

        // traverse through pre-order seq & build tree
        for(; preIdx < preorder.size(); preIdx++) {

            // (1) if traversed node != in-order -> build left child
            if(preOrdPendingNode.top()->val != inorder[ inIdx ]) {
                // new left child
                TreeNode *leftChild = new TreeNode( preorder[ preIdx ] );
                preOrdPendingNode.top()->left = leftChild;

                // add left child to pending (need to check childs of the newly build node)
                preOrdPendingNode.push( leftChild );
                continue;
            }

            // (2) if traversed node = in-order -> leftest child -> pre-order trace back to right-child pending nodes
            TreeNode *nodeNeedRightChild;
            do {
                // trace back along pending nodes (nodes may need right child)
                nodeNeedRightChild = preOrdPendingNode.top();
                preOrdPendingNode.pop();

                // in-order index++ -> in-ord trav to mid node (parent node)
                inIdx++;
            } while(preOrdPendingNode.size() && (preOrdPendingNode.top()->val == inorder[ inIdx ]) );

            // (3) after tracing back -> build right child
            // build right child
            TreeNode *rightChild = new TreeNode( preorder[ preIdx ] );
            nodeNeedRightChild->right = rightChild;

            // add right child to pending (need to chec childs of the newly build node)
            preOrdPendingNode.push( rightChild );
        }
...
## V4：DFS + Recursive Traverse (不容易懂但必須練習)
### V4.1: GPT 修改出來再調整的
- **概念：透過 stopByInOrd + stopVal 來決定 pre-order 是否要換方向 traverse**
	- Pre-order Traverse Left Childs 的時候： 後續一定有某個位置要退回到連結 right child 的地方（或是最後一個 left child 連結 right child）
		- -> traverse to leftest child 後，往下繼續走 left child -> *inorder value = stop value (leftest child node value) : 停止 create left child*
		- 一旦發生 traverse to leftest child 退回
			- in-order index ++
			- **然後試著走 right child**
	- Pre-order Traverse Right Child 的時候：要如何知道當前 node 沒有 right child，還是當前 node 有 right child 對應到 preorder\[ preordIdx \] 的地方 **?**
		- 如果當下需要 pre-order track back (而不是連結到 right child) -> 往下繼續走 right child 發生「上一層帶下來的 stop val，走下一層 right child 發生 inorder value = stop value from prev layer」
		- 這個意思是指「**inorder seq 在目前的 sub-tree 已經走完了，不需要繼續往下走 right child**」
		- 這個發生在「**inorder 走完 left sub-tree 後，退回到 upper layer mid node (parent node)**，這時候 upper layer node value (parent noe value) = inord[ inIdx ]」
- **簡單來說是這樣**
	- Inorder 可以切分成 {left sub tree0} {root0} {right sub tree0} -> {  {left sub tree0} {root0} {right sub tree0} = *upper layer sub tree1* } { *upper layer root 1* } {*upper layer right sub tree 1* }
	- preorder 和 inorder 對位的時候
		- **抓出 in-ord sub root0 的位置：作為 traverse left sub tree0 end**
		- **抓出 in-ord upper layer root1 的位置：作為 traverse right sub tree0 end**
```Cpp
class Solution {
private:
    TreeNode* inOrdTravBuildTree(auto &preorder, int &preIdx, auto &inorder, int &inIdx, bool stopByInOrd, int stopVal) {

        // if preorder traversed to end
        if (preIdx >= (int)preorder.size()) return nullptr;

        // if preorder DFS on stop point
        if (stopByInOrd && inorder[inIdx] == stopVal) return nullptr;

        // build new node
        TreeNode* node = new TreeNode(preorder[preIdx++]);

        // traverse left & build node
        node->left = inOrdTravBuildTree(preorder, preIdx, inorder, inIdx, true, node->val);

        // trace back -> inorder index update
        ++inIdx;

        // traverse right & build node
        node->right = inOrdTravBuildTree(preorder, preIdx, inorder, inIdx, stopByInOrd, stopVal);
...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        int preIdx = 0, inIdx = 0;
        return inOrdTravBuildTree(preorder, preIdx, inorder, inIdx, false, 0);
    }
};
```
...
- Layer0 - inOrdTravBuildTree( preIdx 0, inIdx 0, stopByInOrd = 0, stopVal = 0)
	- Build Node 3
	- node->left = Layer1 - L: inOrdTravBuildTree( preIdx 1, inIdx 0, stopByInOrd = 1, stopVal = 3)
		- Build Node 9
		- node->left = Layer2 - L: inOrdTravBuildTree( preIdx 2, inIdx 0, stopByInOrd = 1, stopVal = 9)
			- stopVal = inorder[ InIdx ] -> return null
		- inIdx++ -> inOrd[1]  =3
		- node->right = Layer2 - R: inOrdTravBuildTree( preIdx 2, inIdx 1, stopByInOrd = 1, stopVal = 3)
			- stopVal = inorderf[ InIdx ] -> return null
		- return node
	- inIdx++ -> inOrd[2] = 15
	- node->right = Layer1 - R: inOrdTravBuildTree( preIdx 2, inIdx 2, stopByInOrd = 0, stopVal = 0)
		- Build Node 20
		- node->left = Layer2 - L: inOrdTravBuildTree( preIdx 3, inIdx 2, stopByInOrd = 1, stopVal = 20)
			- Build Node 15
			- node->left = Layer3 - L: inOrdTravBuildTree( preIdx 4, inIdx 2, stopByInOrd = 1, stopVal = 15)
				- stopVal = inorder[ InIdx ] -> return null
			- inIdx++ -> inOrd[3] = 20
			- node->right = Layer3 - R: inOrdTravBuildTree( preIdx 4, inIdx 3, stopByInOrd = 1, stopVal = 20)
				- stopVal = inorder[ inIdx ] -> return null


### V4.2: 複習2nd 時候寫的 - 自己理解過 + 註解的版本
```Cpp
class Solution {
private:
    TreeNode* buildTreeByPreorder(auto &preorder, int &preIdx, auto &inorder, int &inIdx, int parentNodeValid, int parentNodeVal) {
        // if preorder seq traversed finish
        if(preIdx >= preorder.size()) return nullptr;

        // check whether the current sub-tree is finished
        // (1) build left sub-tree -> stop at inorder seq[i] = root node of sub-tree
        // (2) build right sub-tree -> stop at inorder seq[i] = parent node of root
        if(parentNodeValid && (inorder[ inIdx ] == parentNodeVal)) return nullptr;

        // build node through preorder sequence
        TreeNode *node = new TreeNode( preorder[ preIdx++ ] );

        // build left-side of sub-tree
        // -> current node is the root of the sub-tree -> build left sub-tree until in-ord seq[i] = root (cur node)
        node->left = buildTreeByPreorder(preorder, preIdx, inorder, inIdx, true, node->val);

        // in-order sequence update after tracing back from left child
        inIdx++;

        // build right-side of sub-tree
        // -> stop setting delivered from parent node -> parent node is the stop node of right-sub tree build
        // -> build right sub-tree until in-ord seq[i] = parent node val
        node->right = buildTreeByPreorder(preorder, preIdx, inorder, inIdx, parentNodeValid, parentNodeVal);

        // return node when left & right sub-tree build finish
        return node;
    }
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        int preIdx = 0, inIdx = 0;
        return buildTreeByPreorder(preorder, preIdx, inorder, inIdx, false, 0);
    }
};
```
- **StopByInOrd + stopByValue 的意義：起始是看 parent node 是否為 valid -> 如果 parent node valid，則拿 parent node value 比對 inorder seq[i]**
	-  -> **改名為 parentNodeValid & parentNodeVal**