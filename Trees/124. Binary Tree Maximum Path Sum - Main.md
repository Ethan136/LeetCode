...
# Answer：DFS + Recursive
## V1：複習 1st 時候寫的 - 這個實作比較簡潔，比較好
...
## V1.5：複習 2nd 時候寫的 - 和 V1 類似，重新想到的寫法，這個也可以（看之後哪個比較直覺想到）
```Cpp
class Solution {
    int maxPathSumUnder(TreeNode* node, int &maxSingleSidePathFromNode) {

        if(!node) {
            maxSingleSidePathFromNode = 0;
            return 0;
        }

        int maxSingleSideFromLeftChild = 0;
        int maxPathUnderLeftChild = maxPathSumUnder(node->left, maxSingleSideFromLeftChild);
        if(maxSingleSideFromLeftChild < 0) maxSingleSideFromLeftChild = 0;

        int maxSingleSideFromRightChild = 0;
        int maxPathUnderRightChild = maxPathSumUnder(node->right, maxSingleSideFromRightChild);
        if(maxSingleSideFromRightChild < 0) maxSingleSideFromRightChild = 0;

        maxSingleSidePathFromNode = node->val + max(maxSingleSideFromLeftChild, maxSingleSideFromRightChild);
        int maxPathWithNode = node->val + maxSingleSideFromLeftChild + maxSingleSideFromRightChild;

        int maxPathUnderNode = maxPathWithNode;
        if(node->left) maxPathUnderNode = max(maxPathUnderNode, maxPathUnderLeftChild);
        if(node->right) maxPathUnderNode = max(maxPathUnderNode, maxPathUnderRightChild);
        return maxPathUnderNode;
    }
public:
    int maxPathSum(TreeNode* root) {
        int maxSingleSidePathFromNode;
        return maxPathSumUnder(root, maxSingleSidePathFromNode);
    }
};
```

## V2：複習 2nd 時候寫的 - 這個回傳資訊太多，實作複雜很多，比較容易出錯
```Cpp
class Solution {
    int maxPathSumUnder(TreeNode* node, int &maxPathNodeLeftSide, int &maxPathNodeRightSide) {

        // if not valid node
        if(!node) {
            maxPathNodeLeftSide = 0, maxPathNodeRightSide = 0;
            return 0;
        }

        // init result to contains current node
        int maxPathWithNode = node->val;
        maxPathNodeLeftSide = node->val, maxPathNodeRightSide = node->val;

        // traverse left child
        int maxPathNodeLeftSide_L = 0, maxPathNodeRightSide_L = 0;
        int maxPathSumUnderLeftChild = maxPathSumUnder(node->left, maxPathNodeLeftSide_L, maxPathNodeRightSide_L);

        // update cond from current node to left child
        int maxPathFromLeftChild = max(maxPathNodeLeftSide_L, maxPathNodeRightSide_L);
        maxPathNodeLeftSide += max(0, maxPathFromLeftChild);
        maxPathWithNode += max(0, maxPathFromLeftChild);

        // traverse right child
        int maxPathNodeLeftSide_R = 0, maxPathNodeRightSide_R = 0;
        int maxPathSumUnderRightChild = maxPathSumUnder(node->right, maxPathNodeLeftSide_R, maxPathNodeRightSide_R);

        // update cond from current node to right child
        int maxPathFromRightChild = max(maxPathNodeLeftSide_R, maxPathNodeRightSide_R);
        maxPathNodeRightSide += max(0, maxPathFromRightChild);
        maxPathWithNode += max(0, maxPathFromRightChild);

        // compare max path under (or including) node
        if(node->left) maxPathWithNode = max(maxPathWithNode, maxPathSumUnderLeftChild);
        if(node->right) maxPathWithNode = max(maxPathWithNode, maxPathSumUnderRightChild);

        // finish
        return maxPathWithNode;
    }
public:
    int maxPathSum(TreeNode* root) {
        if(!root) return 0;

        int maxPathNodeLeftSide, maxPathNodeRightSide;
        return maxPathSumUnder(root, maxPathNodeLeftSide, maxPathNodeRightSide);
    }
};
```

# Answer：Iteration（練習用）
