...
## BFS V3: 複習1st - string only - 優化記憶體，簡化 serialized string
```Cpp
#define SERIALIZE_CHILD_NULL_CHAR       ('N')
#define SERIALIZE_NODE_VAL_END_CHAR     ('!')
class Codec {
...
class Codec {
private:
    TreeNode* deserializeCreateOneNode(auto &data, int &idx) {
        // check if the node is nullptr
        if(data[idx++] == 'n') return nullptr;

        // get root value str
        int valStrLen = 0;
        for(int i = idx; i < data.size(); i++) {
            if( (data[i] == 'n') || (data[i] == 'v') ) break;
            valStrLen++;
        }

        // parse node value str
        string valStr = data.substr(idx, valStrLen);

        // update idx after parsing value str
        idx += valStrLen;
...
public:
    ...
    // Encodes a tree to a single string.
    string serialize(TreeNode* root) {
        queue<TreeNode*> thisLayerNode, nextLayerNode;
        thisLayerNode.push(root);

        string result = "";
        while(thisLayerNode.size()) {

            while(thisLayerNode.size()) {
                TreeNode *node = thisLayerNode.front();
                thisLayerNode.pop();
                ...
                result += "v" + to_string(node->val);
                nextLayerNode.push(node->left);
                nextLayerNode.push(node->right);
            }

            swap(thisLayerNode, nextLayerNode);
        }
    ...
    // Decodes your encoded data to tree.
    TreeNode* deserialize(string data) {
        // deserialize parsing index
        int idx = 0;

        // create root
        TreeNode *root = deserializeCreateOneNode(data, idx);
        ...
        while(thisLayerNode.size()) {
            ...
            while(thisLayerNode.size()) {
                ...
                if(!node) continue;
                node->left = deserializeCreateOneNode(data, idx);
                node->right = deserializeCreateOneNode(data, idx);
...
private:
    ...
    TreeNode* deserialize(string &data, int &idx) {
        ...
        // null ptr (or the char should be 'v')
        if(data[idx++] == 'n') return nullptr;
        ...
        // get value
        int value = stoi( data.substr(idx, valStrLen) );
        idx += valStrLen;
