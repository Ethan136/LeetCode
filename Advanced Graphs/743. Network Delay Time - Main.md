...
public:
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
		...
		vector<vector<int>> timeIdxStartFromNode(n+1);
		for(int i = 0; i < times.size(); i++) timeIdxStartFromNode[ times[i][0] ].push_back(i);

		auto cmpNodeReachTime = [](auto &a, auto &b) {return a.time > b.time;};
		priority_queue<timeToNode, vector<timeToNode>, decltype(cmpNodeReachTime)> nodeMinReachTimeHeap;
		nodeMinReachTimeHeap.push({.nodeIdx = k, .time = 0});

		vector<int> nodeMinReachTimePreview(n+1, INT_MAX);
        nodeMinReachTimePreview[k] = 0;

		unordered_map<int, int> nodeReachTime;
		while(nodeReachTime.size() < n) {
			// get the min reach node
			if(nodeMinReachTimeHeap.empty()) return -1;
			auto [reachNode, curReachTime] = nodeMinReachTimeHeap.top();
			nodeMinReachTimeHeap.pop();

			// update node reach time
			if(nodeReachTime.count(reachNode) && (curReachTime > nodeReachTime[reachNode])) continue;
			nodeReachTime[reachNode] = curReachTime;

			// push node delivery from current node
			for(int timeIdx : timeIdxStartFromNode[reachNode]) {
				// delovery info
				int nextReachNode = times[ timeIdx ][1];
				int nextReachTime = times[ timeIdx ][2] + curReachTime;

				// forward trim-branch before push condition to heap
				// (1) cmp to the preview min reach time
				if(nextReachTime >= nodeMinReachTimePreview[nextReachNode]) continue;
				nodeMinReachTimePreview[nextReachNode] = nextReachTime;
				// (2) push to heap
				nodeMinReachTimeHeap.push({.nodeIdx = nextReachNode, .time = nextReachTime});
			}
		}

		// finish
		int maxReachTime = 0;
		for(auto &[nodeIdx, reachTime] : nodeReachTime) maxReachTime = max(maxReachTime, reachTime);		
		return maxReachTime;
    }
};
```