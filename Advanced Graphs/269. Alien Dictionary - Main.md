...
## V2：複習2nd - 模板化寫法 - 末端點往前走
```Cpp
class Solution {
public:
    string foreignDictionary(vector<string>& words) {
	    // if words is not enough to sort out order
	    if(words.size() == 1) return words[0];

		// if any word includes the next word
		for(int i = 0; i < (words.size()-1); i++) {
            int nextWordLen = words[i+1].size();
            if((words[i].size() > nextWordLen) && (words[i].substr(0, nextWordLen) == words[i+1])) {
                return "";
            }
        }

	    // order relation ship
	    unordered_map<char, unordered_set<char>> charToGreaterChar;
        // (1) include all words
        for(string &word : words) {
            for(char c : word) charToGreaterChar[c] = {};
            if(charToGreaterChar.size() == 26) break;
        }
        // (2) order a > b
	    for(int i = 0; i < (words.size()-1); i++) {

		    // check difference between two words
		    for(int c = 0; (c < words[i].size()) && (c < words[i+1].size()); c++) {
			    // skip the identical char parts
			    if(words[i][c] == words[i+1][c]) continue;
			
			    // record the diff char part: next char is lower, current char is greater
				charToGreaterChar[ words[i+1][c] ].insert( words[i][c] );
                break;
		    }
	    }
		...
		// find the char there's no char before
	    while(charToGreaterChar.size()) {
            char charWithNoBefore = '#';
		    for(auto &[chr, chrBeforeSet] : charToGreaterChar) {
			    if(chrBeforeSet.size()) continue;
			    charWithNoBefore = chr;
			    break;
		    }

			if(charWithNoBefore == '#') return "";
			charOrdSeq += charWithNoBefore;

			charToGreaterChar.erase(charWithNoBefore);
            for(auto &[chr, chrBeforeSet] : charToGreaterChar) {
			    if(!chrBeforeSet.count(charWithNoBefore)) continue;
			    chrBeforeSet.erase(charWithNoBefore);
		    }
	    }
