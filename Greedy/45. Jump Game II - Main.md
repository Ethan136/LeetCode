...
public:
    int jump(vector<int>& nums) {
	    // if start on ending position
	    if(nums.size() <= 1) return 0;
	
		// init result
		int jumpCnt = 0;

		// check jump traverse min cnt
	    int nextJumpBestStartIdx = -1;
	    int nextJumpBestEndIdx = -1;
	    for(int i = 0; i < nums.size(); i = nextJumpBestStartIdx) {
			// update jump cnt
		    jumpCnt++;

		    // check if current jump can reach the last index of array
		    if((i + nums[i]) >= (nums.size() - 1)) break;

		    // select the jump destination of this term
		    for(int jumpCnt = 1; jumpCnt <= nums[i]; jumpCnt++) {
			    // start index of next jump
			    int nextJumpStartIdx = i + jumpCnt;

				// end index of next jump
			    int nextJumpMaxEndIdx = nextJumpStartIdx + nums[nextJumpStartIdx];

				// select the index which can jump further as the current destination
			    if(nextJumpMaxEndIdx <= nextJumpBestEndIdx) continue;
			    nextJumpBestStartIdx = nextJumpStartIdx;
				nextJumpBestEndIdx = nextJumpMaxEndIdx;
		    }
	    }
