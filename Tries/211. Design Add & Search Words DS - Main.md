...
# Answer：複習2nd - 模板化變體 - Recursive - Iteration Hybrid
```Cpp
class WordDictionary {
private:
    struct Node {
...
private:
    ...
    bool travAndCheckWord(Node* node, string &word, int to_idx) {
        // check node & index valid
        if(!node) return false;
        if( (to_idx < 0) || (to_idx > word.size()) ) return false;

        // keep checking word[i] != '.'
        for(; (to_idx < word.size()) && (word[to_idx] != '.'); to_idx++) {
            node = node->nextNode[ word[to_idx] - 'a' ];
            if(!node) return false;

            if(to_idx == (word.size() - 1)) return node->endWord;
        }

        // try word[i] as 'a' to 'z'
        for(char toChar = 'a'; toChar <= 'z'; toChar++) {
            Node* nodeOfChar = node->nextNode[ toChar - 'a' ];
            if(!nodeOfChar) continue;

            if(to_idx == (word.size() - 1)) {
                if(!nodeOfChar->endWord) continue;
                return true;
            }

            if(travAndCheckWord(nodeOfChar, word, to_idx+1)) return true;
        }
        return false;
    }
...
public:
    ...
    bool search(string word) {
        return travAndCheckWord(m_root, word, 0);
    }
};
