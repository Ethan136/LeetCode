...
### V4.2 複習2nd：使用模板架構來寫
```Cpp
#define GET_IDX3(idx1, idx2) (idx1 + idx2 + 1)
class Solution {
public:
    bool isInterleave(string s1, string s2, string s3) {
        if((s1.size() + s2.size()) != s3.size()) return false;
        ...
        // match cond
        // [idx1=-1 / idx2=-1, 0, 1, 2, 3, 4..]
        // [idx1= 0 / idx2=-1, 0, 1, 2, 3, 4..]
        // [idx1= 2 / idx2=-1, 0, 1, 2, 3, 4..]
        // match cond[idx1, idx2] = 
        // (1) (match cond[idx1-1, idx2] & s1[idx1] == s3[idx3])
        // (2) (match cond[idx1, idx2-1] & s2[idx2] == s3[idx3])

        // [match cond when of idx1 = certain with idx2 = -1 to end]
        if(s1.size() < s2.size()) swap(s1, s2);
        vector<bool> matchCondToIdx2(s2.size()+1, false);

        // [initial condition idx1 = -1]
        // (0) idx1 = -1
        int idx1 = -1;

        // (1) idx2 = -1
        int idx2 = -1;
        matchCondToIdx2[idx2 + 1] = true;

        // (2) idx2 = 0 to end
        for(idx2 = 0; idx2 < s2.size(); idx2++) {
            if(s2[idx2] != s3[GET_IDX3(idx1, idx2)]) break;
            matchCondToIdx2[idx2 + 1] = true;
        }

        // [check from idx1=0 to end]
        for(idx1 = 0; idx1 < s1.size(); idx1++) {
            vector<bool> matchCondToIdx2_next(s2.size()+1, false);

            // (1) check idx1 = valid & idx2 = -1
            idx2 = -1;
            matchCondToIdx2_next[idx2 + 1] = (matchCondToIdx2[idx2 + 1] && (s1[idx1] == s3[GET_IDX3(idx1, idx2)]));
    
            // (2) check idx1 = valid & idx2 = valid
            for(idx2 = 0; idx2 < s2.size(); idx2++) {

                matchCondToIdx2_next[idx2 + 1] = (matchCondToIdx2_next[idx2-1 + 1] && (s2[idx2] == s3[GET_IDX3(idx1, idx2)])) ||
                                             (matchCondToIdx2[idx2 + 1] && (s1[idx1] == s3[GET_IDX3(idx1, idx2)]));
            }

            swap(matchCondToIdx2, matchCondToIdx2_next);
        }

        return matchCondToIdx2.back();
    }
};
```