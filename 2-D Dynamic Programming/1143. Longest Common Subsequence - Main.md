...
public:
    int longestCommonSubsequence(string text1, string text2) {
        // common sub-seq len comSubSeqLen[idx1][idx2]
        // idx1: try idx after of text1
        // idx2: try idx after of text2
        // comSubSeqLen[idx1][idx2]
        // if(text1[idx1] == text2[idx2]) -> 1+ comSubSeqLen[idx1+1][idx2+1]
        // else -> try idx after (idx1+1, idx2) & (idx1, idx2+1) & (idx1+1, idx2+1)
        //      -> max(comSubSeqLen[idx1+1][idx2+1], max(comSubSeqLen[idx1+1][idx2], comSubSeqLen[idx1][idx2+1]))
        // -> comSubSeqLen[idx2] (on idx1) needs
        // (1)(2) comSubSeqLen[idx2+1] (idx1 & idx1+1)
        // (3) comSubSeqLen[idx2] (next on idx1+1)

        // select shorter one aw text2 (append one with "out of range (zero match num)")
        if(text1.size() < text2.size()) swap(text1, text2);
        vector<int> comSubSeqLen_idx1(text2.size()+1, 0);
        vector<int> comSubSeqLen_idx1Next(text2.size()+1, 0);

        // check from right down corner back to top left corner
        for(int idx1 = text1.size() - 1; idx1 >= 0; idx1--) {
            swap(comSubSeqLen_idx1, comSubSeqLen_idx1Next);

            for(int idx2 = text2.size() - 1; idx2 >= 0; idx2--) {
                if(text1[idx1] == text2[idx2]) {
                    comSubSeqLen_idx1[idx2] = 1 + comSubSeqLen_idx1Next[idx2+1];
                }
                else {
                    comSubSeqLen_idx1[idx2] = max( comSubSeqLen_idx1Next[idx2+1], 
                                                    max( comSubSeqLen_idx1Next[idx2], comSubSeqLen_idx1[idx2+1])
                                                    );
                }
            }
        }

        // return max possible matching index after text1[0] / text2[0]
        return comSubSeqLen_idx1[0];
