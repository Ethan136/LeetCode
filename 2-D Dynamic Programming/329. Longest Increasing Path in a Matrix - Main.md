...
## V3：複習2nd - Recursive + 2D DP
```Cpp
class Solution {
private:
    const int m_dir[4][2] = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};

    int longestIncPath(auto &matrix, int r, int c, auto &maxIncPathRecord) {
        // if current position had been checked before
        if(maxIncPathRecord[r][c] != -1) return maxIncPathRecord[r][c];

        // backup matrix grid & mark as traversed
        int gridNum = matrix[r][c];
        matrix[r][c] = INT_MIN;

        // only go through valid grids
        int maxPathCnt = 1;
        for(int d = 0; d < 4; d++) {
            int r2 = r + m_dir[d][0];
            int c2 = c + m_dir[d][1];
            ...
            if((r2 < 0) || (r2 >= matrix.size())) continue;
            if((c2 < 0) || (c2 >= matrix[r2].size())) continue;
            if(matrix[r2][c2] <= gridNum) continue;

            maxPathCnt = max(maxPathCnt, 1 + longestIncPath(matrix, r2, c2, maxIncPathRecord));
        }
        ...
        // finish
        maxIncPathRecord[r][c] = maxPathCnt;
        return maxPathCnt;
    }
public:
    int longestIncreasingPath(vector<vector<int>>& matrix) {

        vector<vector<int>> maxIncPathRecord(matrix.size(), vector<int>(matrix[0].size(), -1));

        int maxIncPath = 0;
        for(int r = 0; r < matrix.size(); r++) {
            for(int c = 0; c < matrix[r].size(); c++) {
                maxIncPath = max(maxIncPath, longestIncPath(matrix, r, c, maxIncPathRecord));
            }
        }
        return maxIncPath;
    }
