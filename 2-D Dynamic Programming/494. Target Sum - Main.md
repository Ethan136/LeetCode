...
## V5：複習1st - Iteraion Top Down + 1D DPx2 + DP 儲存從 map 改成 vector
...
## V6：複習2nd - Iteration Top Down + 1D DPx2 + 隨時計算 current max sum 以減少計算範圍
```Cpp
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        // method(target) = method(target - nums.end) + method(target + nums.end)
        // method(target - nums.end) = method((target - nums.end) - nums.(end-1)) + method((target - nums.end) +  nums.(end-1))
        ...
        // check if target within boundary
        if(abs(target) > maxSum) return 0;

        // record method cnt of all possible amount -> shift amount from [-maxSum , maxSum] -> [0, 2maxSum]
        vector<int> shiftedAmountMethod(2*maxSum + 1, 0);

        // initial cond -> use +1 to include nums[0] = 0 -> two methods
        int curSumMax = abs(nums[0]);
        shiftedAmountMethod[nums[0] + maxSum] += 1;
        shiftedAmountMethod[-nums[0] + maxSum] += 1;

        // expand amount along nums idx
        for(int idx = 1; idx < nums.size(); idx++) {
            // current num
            int absNum = abs(nums[idx]);

            // next term of calculation result (alike BFS double stack -> next stack init as empty)
            vector<int> shiftedAmountMethodNext(2*maxSum + 1, 0);

            // cond2: idx > 0 -> expand previous method cnt to next method cnt
            for(int i = -curSumMax; i <= curSumMax; i++) {
                if(shiftedAmountMethod[i + maxSum] == 0) continue;
                shiftedAmountMethodNext[i + absNum + maxSum] += shiftedAmountMethod[i + maxSum];
                shiftedAmountMethodNext[i - absNum + maxSum] += shiftedAmountMethod[i + maxSum];
            }

            // deliver from previous method cnt
            swap(shiftedAmountMethod, shiftedAmountMethodNext);

            // update boundary of next check
            curSumMax += absNum;
        }

        return shiftedAmountMethod[target + maxSum];
    }
};
```