...
## V3 - 複習1st：一個 stack 同時儲存 value / min value
```Cpp
class MinStack {
private:
...
## V4 - 複習2nd：兩個 stack 存min / all value (比較慢，但直覺)
```C
class MinStack {
private:
    stack<int> m_minStack;
...
public:
    ...
    void push(int val) {
        m_stack.push(val);
        if(m_minStack.empty() || val <= m_minStack.top()) m_minStack.push(val);
    }
    
    void pop() {
		int popVal = m_stack.top();
		m_stack.pop();
		if(m_minStack.top() == popVal) m_minStack.pop();
    }
