...
public:
    int largestRectangleArea(vector<int>& heights) {
		...
		// append heights with zero height for terminate condition
	    heights.push_back(0);

		// height idx still not determined area
        stack<int> pendingHeightIdx;
		...
		for(int i = 0; i < heights.size(); i++) {
			// check pending heights
			while(pendingHeightIdx.size() && (heights[ pendingHeightIdx.top() ] > heights[i])) {
				int h_idx = pendingHeightIdx.top();
				pendingHeightIdx.pop();

				int widthRight = i - h_idx;
				int widthLeft = h_idx - (pendingHeightIdx.size() ? (pendingHeightIdx.top()+1) : 0);

				int h_area = heights[ h_idx ] * (widthLeft + widthRight);
				maxArea = max( maxArea, h_area);
			}

			// current height push to pending stack
            // (still not know the next height is larger or lower)
			pendingHeightIdx.push(i);
		}
...
```
- 注意：**如果 current height = previous pending height -> 也要 push 到 stack 裡面**
	- 否則會發生 [0 , 1 , 0 , 1] -> index  = 2 位置的 height = 0，會被拿來和 index 0 的 1 合併
	-  -> stack 裡面只有 index 0，後續檢查 index 3 的 1，會誤判 **前一個 low 再 index 0 而不是 index 2，導致面積計算錯誤** 