...
## V2 - 複習2nd：DFS + Recursive
```Cpp
class Solution {
private:
    void generateParenthesis(int leftRemainNum, int rightRemainNum, string &curResult, vector<string> &resultSet) {
        // invalid condition
        if(leftRemainNum < 0) return;
        if(rightRemainNum < 0) return;
        if(leftRemainNum > rightRemainNum) return;
        ...
        // traverse with left
        curResult.push_back('(');
        generateParenthesis(leftRemainNum-1, rightRemainNum, curResult, resultSet);
        curResult.pop_back();

        // traverse with right
        curResult.push_back(')');
        generateParenthesis(leftRemainNum, rightRemainNum-1, curResult, resultSet);
        curResult.pop_back();
    }
public:
    vector<string> generateParenthesis(int n) {
        ...
        generateParenthesis(n, n, curResult, resultSet);
        return resultSet;
    }
};
```