...
class Solution {
private:
    void getTargetCombinationFromIdx(auto &nums, int startIdx, int target, auto &curComb, auto &allCombination) {
        if(target < 0) {
            return;
        }

        if(target == 0) {
            allCombination.push_back(curComb);
            return;
        }

        for(int i = startIdx; i < nums.size(); i++) {
            int numRepeatCntMax = (target / nums[i]);
            for(int r = 1, repeatCnt = 0; r <= numRepeatCntMax; r++) {
                repeatCnt = r;
                while(repeatCnt--) curComb.push_back(nums[i]);

                getTargetCombinationFromIdx(nums, i+1, target - r*nums[i], curComb, allCombination);

                repeatCnt = r;
                while(repeatCnt--) curComb.pop_back();
            }
        }
    }
public:
    vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
        ...
        getTargetCombinationFromIdx(candidates, 0, target, curComb, allCombination);
