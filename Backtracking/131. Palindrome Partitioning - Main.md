...
class Solution {
private:
    bool checkSubStrPalindrome(string &s, int l, int r) {
        if(l < 0) return false;
    ...
    void getPartitionFromIdx(string &s, int startIdx, auto &curPartition, auto &resultPartitions) {
        if(startIdx == s.size()) {
            resultPartitions.push_back(curPartition);
            return;
        }

        for(int l = startIdx, r = startIdx; r < s.size(); r++) {

            if(!checkSubStrPalindrome(s, l, r)) continue;
            curPartition.push_back(s.substr(l, r-l+1));

            getPartitionFromIdx(s, r+1, curPartition, resultPartitions);
            curPartition.pop_back();
        }
    }
public:
    vector<vector<string>> partition(string s) {
        vector<string> curPartition; curPartition.reserve(s.size());
        vector<vector<string>> resultPartitions;

        getPartitionFromIdx(s, 0, curPartition, resultPartitions);
