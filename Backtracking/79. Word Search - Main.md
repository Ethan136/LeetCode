...
## V3：複習2nd - 模板化寫法（使用 2D Graph 的 board 模板）
```Cpp
#define CHAR_MARK_TRAVERSED ('#')
class Solution {
private:
    bool travCheckWord(auto &board, int row, int col, auto &word, int idx) {
        ...
        char charBkp = board[row][col];
        board[row][col] = CHAR_MARK_TRAVERSED;
        
        bool checkPass = false;
        int dir[4][2] = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};
        for(int i = 0; i < 4; i++) {
            checkPass = travCheckWord(board, row + dir[i][0], col + dir[i][1], word, idx+1);
            if(checkPass == true) break;
        }

        board[row][col] = charBkp;
        return checkPass;
    }
public:
    bool exist(vector<vector<char>>& board, string word) {
        for(int row = 0; row < board.size(); row++) {
            for(int col = 0; col < board[row].size(); col++) {
                if(travCheckWord(board, row, col, word, 0)) return true;
            }
        }
        return false;
    }
