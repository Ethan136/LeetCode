...
## V3 - DFS 複習2nd 時候寫的 - 模板化寫法
```Cpp
class Solution {
private:
    string digitToChar[10] = {
        "", // 0
        "", // 1
        "abc",
        "def",
    ...
    void travFindStr(auto &digits, int startIdx, auto &curResult, auto &allResultSet) {
        if(startIdx >= digits.size()) {
            allResultSet.push_back(curResult);
            return;
        }

        for(char c : digitToChar[ digits[startIdx] - '0' ]) {
            curResult.push_back(c);
            travFindStr(digits, startIdx+1, curResult, allResultSet);
            curResult.pop_back();
        }
    }
public:
    vector<string> letterCombinations(string digits) {
        string curResult; curResult.reserve(digits.size());
        vector<string> allResultSet;

        travFindStr(digits, 0, curResult, allResultSet);
        return allResultSet;
    }
};
```
