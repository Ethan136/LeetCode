...
# Answer：複習2nd - DFS Traverse 模板
```Cpp
#define _NUM_MARK_TRAV_ (INT_MIN)
class Solution {
private:
    void getAllPermute(auto &nums, int needNum, auto &curResult, auto &allResultSet) {
        if(needNum == 0) {
        ...
        for(int i = 0; i < nums.size(); i++) {
            if(nums[i] == _NUM_MARK_TRAV_) continue;
            int numOfIdx = nums[i];

            nums[i] = _NUM_MARK_TRAV_;
            curResult.push_back(numOfIdx);

            getAllPermute(nums, needNum-1, curResult, allResultSet);
...
public:
    vector<vector<int>> permute(vector<int>& nums) {
        ...
        vector<vector<int>> allResultSet;

        getAllPermute(nums, nums.size(), curResult, allResultSet);
        return allResultSet;
    }
};
```