...
# Answer V2：複習1st - 使用額外 vector 儲存 current result - 比較好想
...
class Solution {
private:
    void getSubSets(auto &nums, int startIdx, int subSetSize, auto &allSubSets, auto &curSubSet) {

        // need no number -> finish point
        if(subSetSize == 0) {
            allSubSets.push_back(curSubSet);
            return;
        }

        // add number to sub-set
        for(int i = startIdx; i < nums.size(); i++) {
            curSubSet.push_back(nums[i]);
            getSubSets(nums, i+1, subSetSize-1, allSubSets, curSubSet);
            curSubSet.pop_back();
        }
    }
public:
    vector<vector<int>> subsets(vector<int>& nums) {
        ...
        // to record the current sub-set
        vector<int> curSubSet; curSubSet.reserve(nums.size());

        // try sub-set with element cnt 0 to N
        for(int subSetCnt = 0; subSetCnt <= nums.size(); subSetCnt++) {
            getSubSets(nums, 0, subSetCnt, allSubSets, curSubSet);
        }
