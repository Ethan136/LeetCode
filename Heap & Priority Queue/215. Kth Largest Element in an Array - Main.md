...
public:
    int findKthLargest(vector<int>& nums, int k) {
        priority_queue<int, vector<int>, greater<int>> m_minHeapLargestK;
        for(auto num : nums) {
            if(m_minHeapLargestK.size() < k) {
                m_minHeapLargestK.push(num);
                continue;
            }

            if(num <= m_minHeapLargestK.top()) {
                continue;
            }

            else {
                m_minHeapLargestK.push(num);
                m_minHeapLargestK.pop();
            }
        }
        return m_minHeapLargestK.top();
    }
};
```