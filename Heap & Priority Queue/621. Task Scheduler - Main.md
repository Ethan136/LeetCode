...
public:
    int leastInterval(vector<char>& tasks, int n) {
        // statistic all tasks
        int taskCnt[26] = {};
        for(char t : tasks) taskCnt[t - 'A']++;

        // use interval (n+1) of one arrangement term
        // (1) use two maxHeaps for arrangement
        auto cmpTaskInfo = [](auto &a, auto &b) {return a.cnt < b.cnt;};
        priority_queue<taskInfo, vector<taskInfo>, decltype(cmpTaskInfo)> thisTermTasks, nextTermTasks;

        // (2) initial cond
        // add all tasks in arrangement arbiter
        for(char t = 'A'; t <= 'Z'; t++) {
            if(taskCnt[t - 'A']) thisTermTasks.push({t, taskCnt[t - 'A']});
        }

        // counter for arrangement intervals
        int optIntervals = 0;

        // (3) process arrangement
        int intervalOfTerm = n+1;
        while(thisTermTasks.size()) {

            // total interval of this term
            for(int i = 0; i < intervalOfTerm; i++, optIntervals++) {

                // all tasks finished
                if(thisTermTasks.empty() && nextTermTasks.empty()) break;

                // can't arrange anymore in this term
                if(thisTermTasks.empty()) continue;

                // do one task from heap
                auto [t, cnt] = thisTermTasks.top(); thisTermTasks.pop();
                cnt--;

                // add to next term if not finished totally
                if(cnt) nextTermTasks.push({t, cnt});
            }

            // transfer all the remained tasks to next term
            while(thisTermTasks.size()) {
                nextTermTasks.push( thisTermTasks.top() );
                thisTermTasks.pop();
            }

            // prepare for next term
            swap(thisTermTasks, nextTermTasks);
        }
