...
## V3: 複習2nd - 資料結構沒優化的流程精簡版本（資料結構比較直覺的版本）
```Cpp
#define MAX_FETCH_POST_NUM  (10)
class Twitter {
private:
    struct postInfo {
        int userId;
    ...
    unordered_map<int, unordered_set<int>> m_userFollow;
    unordered_map<int, vector<postInfo>> m_userTweetList;
    int m_timeStamp;
public:
    Twitter() {
        m_timeStamp = 0;
    ...
    void postTweet(int userId, int tweetId) {
        m_userTweetList[userId].push_back(
            {.userId = userId, .tweetId = tweetId, .timeStamp = m_timeStamp, .storeIdx = (int)(m_userTweetList[userId].size())});
        m_timeStamp++;
    }
    
    vector<int> getNewsFeed(int userId) {
        // post arbiter
        auto cmpPostInfo = [](auto &a, auto &b) {return a.timeStamp < b.timeStamp;};
        priority_queue<postInfo, vector<postInfo>, decltype(cmpPostInfo)> latestPostHeap;

        // add user's latest post to arbiter
        if(m_userTweetList[userId].size()) {
            latestPostHeap.push(m_userTweetList[userId].back());
        }

        // all followee's posts to arbiter
        for(int followeeId : m_userFollow[userId]) {
            if(m_userTweetList[followeeId].empty()) continue;
            latestPostHeap.push(m_userTweetList[followeeId].back());
        }

        // process post scheduling
        vector<int> fetchPosts; fetchPosts.reserve(MAX_FETCH_POST_NUM);
        while(fetchPosts.size() < MAX_FETCH_POST_NUM) {
            if(latestPostHeap.empty()) break;

            auto [followeeId, tweetId, timeStamp, storeIdx] = latestPostHeap.top();
            latestPostHeap.pop();

            fetchPosts.push_back(tweetId);

            if(storeIdx == 0) continue;
            latestPostHeap.push( m_userTweetList[followeeId][storeIdx-1] );
        }
    ...
    void follow(int followerId, int followeeId) {
        m_userFollow[followerId].insert(followeeId);
    }
    
    void unfollow(int followerId, int followeeId) {
        m_userFollow[followerId].erase(followeeId);
    }
};
