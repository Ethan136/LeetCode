...
class KthLargest {
private:
    priority_queue<int, vector<int>, greater<int>> m_minHeapLargeK;
    //TYPE_HEAP(less<int>) m_maxHeapSmallerNum;
    int m_k;
...
public:
    ...
    int add(int val) {
        // (1) less than k num totally
        if(m_minHeapLargeK.size() < m_k) {
            m_minHeapLargeK.push(val);
        }

        // (2) num less than k-th largest
        else if(val < m_minHeapLargeK.top()) {
            //m_maxHeapSmallerNum.push(val);
        }

        // (3) num greater than k-th largest
        // -> update k-the largest
        else {
            m_minHeapLargeK.push(val);
            //m_maxHeapSmallerNum.push( m_minHeapLargeK.top() );
            m_minHeapLargeK.pop();
        }

        // finish
        return m_minHeapLargeK.top();
    }
};
```