...
## V3：一次走一個大方向，走完更新 remain row / col -> 一旦有某個 row / col 走完剩餘0 -> 直接結束 (最直覺簡潔)
```Cpp
class Solution {
private:
    void travInDir(vector<vector<int>> &matrix, int startRow, int startCol, vector<int> dir, int stepNum, 
                                                int &endRow, int &endCol, auto &result) {
        for(int i = 1; i <= stepNum; i++) {
            endRow = startRow + dir[0] * i;
            endCol = startCol + dir[1] * i;
            result.push_back(matrix[endRow][endCol]);
        }
    }
public:
    vector<int> spiralOrder(vector<vector<int>>& matrix) {
        ...
        vector<int> result;
        result.reserve(rowNum * colNum);

        int posRow = 0, posCol = -1;
        while(true) {

            // col low to high
            travInDir(matrix, posRow, posCol, {0, 1}, colNum, posRow, posCol, result);
            if( (--rowNum) == 0) break;

            // row low to high
            travInDir(matrix, posRow, posCol, {1, 0}, rowNum, posRow, posCol, result);
            if( (--colNum) == 0) break;
