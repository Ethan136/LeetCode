...
## Code V2 - 複習1st：寫比較簡潔，code 也比較好懂（看 code）
...
## Code V3 - 複習2nd：直接把 interval 放到 heap 裡面 (比較直覺)
```Cpp
/**
 * Definition of Interval:
 * class Interval {
...
public:
    int minMeetingRooms(vector<Interval>& intervals) {
        // if there's no meeting or only one meeting
        if(intervals.size() <= 1) return intervals.size();

        // sort intervals by start time
        auto cmpItvlStart = [](auto &a, auto &b) {return a.start < b.start;};
        sort(intervals.begin(), intervals.end(), cmpItvlStart);

        // min ending time heap for the ending interval of each rooms
        auto cmpItvlEnd = [](const Interval &a, const Interval &b) {return a.end > b.end;};
        priority_queue<Interval, vector<Interval>, decltype(cmpItvlEnd)> lastItvlOfRooms(cmpItvlEnd);

        // use one room for the earliest interval
        lastItvlOfRooms.push(intervals[0]);

        // check the remained intervals
        for(int i = 1; i < intervals.size(); i++) {
            // get the earlist ending interval of all the used rooms
            auto const &earliestEndItvl = lastItvlOfRooms.top();

            // if ending interval of room is not overlap with intervals[i]
            // use this room for new interval
            if(earliestEndItvl.end <= intervals[i].start) {
                lastItvlOfRooms.pop();
                lastItvlOfRooms.push(intervals[i]);
            }
            // if ending interval of room overlaps with intervals[i]
            // open new room for the new interval
            else {
                lastItvlOfRooms.push(intervals[i]);
            }
        }

        // finish -> size of lastItvlOfRoom = used rooms
        return lastItvlOfRooms.size();
