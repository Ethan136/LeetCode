...
public:
    int eraseOverlapIntervals(vector<vector<int>>& intervals) {
        ...
        // check all intervals and take out the overlapped
        int baseIdx = 0, cmpIdx = 1;
        while(cmpIdx < intervals.size()) {

            // select two intervals from index
            auto &baseItvl = intervals[baseIdx];
            auto &cmpItvl = intervals[cmpIdx];

            // A. if non-overlap
            if(baseItvl[1] <= cmpItvl[0]) {
                baseIdx = cmpIdx;
                cmpIdx++;
                continue;
            }

            // B. if overlap: keep the interval which ends earlier
            // (take out the interval ends later)
            // (0) update remove cnt
            removeCnt++;

            // (1) keep base interval -> update compare interval idx
            if(baseItvl[1] < cmpItvl[1]) {
                cmpIdx++;
            }
            // (2) keep compare interval idx -> update base interval & cmp interval idx
            else {
                // set compare interval as new base interval
                baseIdx = cmpIdx;

                // select next interval to compare
                cmpIdx++;
            }
        }
