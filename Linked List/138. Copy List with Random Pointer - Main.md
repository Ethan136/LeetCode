...
class Solution {
private:
    Node* traverseAndCopyNode(Node* originNode, auto &oldNewNodeMap) {
        if(!originNode) return nullptr;
        if(oldNewNodeMap.count(originNode)) return oldNewNodeMap[originNode];

        Node* cloneNode = new Node(originNode->val);
        oldNewNodeMap[originNode] = cloneNode;

        cloneNode->next = traverseAndCopyNode(originNode->next, oldNewNodeMap);
        cloneNode->random = traverseAndCopyNode(originNode->random, oldNewNodeMap);
        return cloneNode;
    }
public:
    Node* copyRandomList(Node* head) {
        unordered_map<Node*, Node*> oldNewNodeMap;
        return traverseAndCopyNode(head, oldNewNodeMap);
    }
};
/*
// Definition for a Node.
