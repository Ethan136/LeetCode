...
private:
    ...
    unordered_map<string, vector<timeAndVal>> m_keyTimeToVal;
public:
    TimeMap() {
    ...
    void set(string key, string value, int timestamp) {
        m_keyTimeToVal[key].push_back(
            {.timestamp = timestamp, .value = value});
    }
    
    string get(string key, int timestamp) {
        if(!m_keyTimeToVal.count(key)) return "";

        auto &timeAndValVec = m_keyTimeToVal[key];
        if(timeAndValVec.empty()) return "";

        if(timestamp < timeAndValVec.front().timestamp) return "";
        if(timeAndValVec.back().timestamp < timestamp) return timeAndValVec.back().value;

        int head, tail, mid;
        for(head = 0, tail = (timeAndValVec.size()-1), mid = (head+tail)/2;
            head < (tail-1);
            mid = (head+tail)/2)
        {
            // match requested timestamp
            if(timeAndValVec[mid].timestamp == timestamp) {
                return timeAndValVec[mid].value;
            }

            // Val(mid) = [Val(head), target)
            if(timeAndValVec[mid].timestamp < timestamp) {
                head = mid;
            }
            // Val(mid) = (target, Val(tail)]
            else {
                tail = mid;
            }
        }

        // (1) tail match time stamp
        // (2) lesss than time stamp (head < tail -> head must not match)
        if(timeAndValVec[tail].timestamp <= timestamp) {
            return timeAndValVec[tail].value;
        }
        // (3) head match time stamp
        // (4) head less than time stamp & tail larget than timestamp
        else {
            return timeAndValVec[head].value;
        }
    }
};
