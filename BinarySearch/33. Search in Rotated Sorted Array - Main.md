...
# Answer：複習2nd - 先把區間切分成兩個可以各自 binary search，再做處理
```Cpp
class Solution {
public:
    int search(vector<int>& nums, int target) {
        // [leftseq, rightseq]
        bool targetInLeftSeq = (nums[0] <= target);

        // linear search: find i = 0 to end idx
        // binary search: mid = (0 + end_idx)/2
        int head, tail, mid;
        for(head = 0, tail = nums.size()-1, mid = (head+tail)/2;
            head < (tail - 1);
            mid = (head+tail)/2) {
            
            // (0) target num is found
            if(nums[mid] == target) return mid;

            // sequence section of mid
            bool midInLeftSeq = (nums[0] <= nums[mid]);

            // (1) if target & mid in different section
            if(targetInLeftSeq != midInLeftSeq) {
                // (a) target in left & mid in right
                if(targetInLeftSeq) tail = mid;
                // (b) target in right & mid in left
                else head = mid;
            }
            // (2) if target & mid in same section
            else {
                // (a) [head, mid, target, tail]
                if(nums[mid] < target) head = mid;
                // (b) [head, target, mid, tail]
                else tail = mid;
            }
        }
